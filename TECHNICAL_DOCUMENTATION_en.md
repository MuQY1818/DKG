...
## 5. Future Work: GNN-based Performance Prediction

The current DKG model relies on explicitly defined rules (e.g., ZPD, knowledge propagation) for updates and recommendations. While this provides excellent interpretability, the model's ability to capture complex, high-order features is limited. Introducing Graph Neural Networks (GNNs) aims to automatically learn vector representations (Embeddings) for students, problems, and skills directly from the graph structure through end-to-end learning, enabling more accurate prediction of student performance.

We have completed a proof-of-concept in the `dkg_mvp/gnn_trainer.py` script. This implementation, based on the **PyTorch Geometric (PyG)** framework, explores the feasibility of using GNNs for link prediction.

### 5.1 Core Workflow

1.  **Heterogeneous Graph Conversion**: The `networkx` graph generated by `dkg_builder` is converted into a PyG `HeteroData` object. This object clearly represents different types of nodes (`student`, `problem`, `skill`) and edges (`solve`, `require`, and their reverse counterparts).

2.  **Model Architecture**:
    *   **Encoder**: A two-layer `SAGEConv` model is used. `SAGEConv` is a powerful graph convolution operator capable of aggregating information from neighboring nodes to generate embeddings for a target node. The encoder's role is to map each node in the graph to a low-dimensional, dense vector space.
    *   **Decoder**: A simple Multi-Layer Perceptron (MLP), defined as the `LinkPredictor`. Its task is to take the representation vectors of a (student, problem) pair and predict the existence of a "solve" link between them, effectively predicting whether the student can solve the problem correctly.

3.  **Training and Evaluation**:
    *   **Task**: The problem is framed as a **link prediction** task. We mask a portion of the `(student, 'solve', problem)` edges and train the model to predict whether these masked edges exist.
    *   **Data Split**: The `solve` interaction edges are split into training, validation, and test sets using an 80:10:10 ratio.
    *   **Loss Function**: `BCEWithLogitsLoss` is used, suitable for binary classification tasks (predicting the `correct` label of the interaction, i.e., success or failure).
    *   **Evaluation Metrics**: `AUC` (Area Under Curve) and `F1-Score` are used to evaluate the model's predictive performance on the validation and test sets.

### 5.2 Results & Outlook

*   **Initial Results**: Experiments have shown that the GNN model achieved an `AUC of 0.707` and an `F1-Score of 0.808` on the test set, validating the effectiveness of this approach.
*   **Application Value**: The **node embeddings** produced by the trained GNN model are highly valuable. These embeddings can replace or supplement the manually engineered features in the DKG (like `mastery_score`) for:
    1.  **More Accurate Recommendations**: Recommending problems based on vector similarity between students and problems.
    2.  **Student Clustering**: Discovering groups of students with similar knowledge structures or learning behaviors.
    3.  **Knowledge Structure Analysis**: Analyzing skill and problem embeddings to uncover implicit knowledge relationships.
*   **Evolution Path**: This provides a clear technical path for the project to evolve from an **interpretable rule-based engine** to a **data-driven intelligent prediction and recommendation system**.

---

## 6. Appendix

### 6.1 Data Source Compatibility
